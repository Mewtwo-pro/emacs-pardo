...
(defun atajos ()
  ...
  (let ((archivo (expand-file-name "atajos.txt" user-emacs-directory)))
    (split-window-below)
    (other-window 1)
    (find-file archivo)))
...
(defun copiar-current-path ()
  (interactive)
  (let ((filename (buffer-file-name)))
    (if filename
        (let ((dirname (directory-file-name (file-name-directory filename))))
          (kill-new dirname)
          (message "Carpeta copiada: %s" dirname))
      (message "¡Ups! Este buffer no está visitando ningún archivo."))))
...
(defun my-a-function ()
	...
	(cond
	 ;(copiar-ruta))
   	((or(= my-toggle-var 1)(= my-toggle-var 2)(= my-toggle-var 5))
	   (setq my-toggle-var 0)
	   (setq mode-select 0)
	   (setq buscar-contex 0)
	   (messages-mode my-toggle-var)
